...
class Sharesprices:
    ...
    def options(self):
        opt_dates = []
        df_tickers = pd.read_csv('tickers_list.csv')[:50]
        for i in df_tickers['Tickers']:
            try:
                opt_dates.append(yf.Ticker(i).options[0])
            except Exception as e:
                opt_dates.append('Not available')
                continue
        return opt_dates
    ...
    def final(self):
        ...
        df_final = pd.DataFrame(list(zip(df_tickers['Tickers'],self.stats()[0],self.stats()[1],self.stats()[2],
                                         ...
                                         self.stats()[8],self.hist_imp_vol(),self.vol_check(),self.options())),
                                columns=['ticker','daily_change','weekly_change','fortnightly_change','monthly_change',
                                         ...
                                         'hist_imp_vol_monthly','HV_IV indicator','opt_dates'])
